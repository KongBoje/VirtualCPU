...
public class Memory {
    ...
    public int get(int index) { // & makes it binary. 0b means that what comes after is binary. 255 = hexadecimal. 00377 = oqtal. also the same as writing 0b0000_0000_0000_0000_0000_0000_1111_1111. Where there are 0s it will be 0, where there are 1s it will be the same number as the one next to the 1s(1-0).
        int value = data[index%64]; // & is binary and: and on every bit in value and 0b1111_1111
        value = value & 255;
        return value;
        //return data[index%64];
    }
    
    public void set(int index, int value) {
        // TODO
        data[index%64] = (byte)(value & 0b1111_1111);
    }
    ...
    public void print(PrintStream out, Cpu cpu, int index){ //on print out it should have 4 spaces == %4d. Have 2 places, space then 4 spaces.
        // should get out as binary number.
        if(cpu.getIp() == index){ //Instruction pointer = IP
            out.printf("==> %2d: %4d %s\t", index, get(index), binary(get(index)));
        } else if(cpu.getSp() == index){
            out.printf("--> %2d: %4d %s\t", index, get(index), binary(get(index)));
        } else {
            out.printf("    %2d: %4d %s\t", index, get(index), binary(get(index)));
        }
    ...
    public void print(PrintStream out, Cpu cpu){ //print out a memory.
        for(int index = 0; index < 16; index++){
            print(out, cpu, index);
            print(out, cpu, 16 + index);
            print(out, cpu, 32 + index);
            print(out, cpu, 48 + index);
            out.println();
        }
    }
}
